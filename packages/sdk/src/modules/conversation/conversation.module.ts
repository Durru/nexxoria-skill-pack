import { ConversationInput, ConversationIntent, ConversationResult } from './conversation.types.js'
...
const detectIntent = (prompt: string): ConversationIntent => {
  const normalized = normalizePrompt(prompt).toLowerCase()

  if (normalized.length < 2) return 'empty_input'
...
const getFirstBullet = (content: string): string | null => {
  return content
    .split('\n')
    .find((line) => line.startsWith('- '))?.slice(2) ?? null
}

const buildSystemSnapshot = (input: ConversationInput) => {
...
const buildMessage = (intent: ConversationIntent, input: ConversationInput): string => {
